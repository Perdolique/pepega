...
import type { WebhookStatus } from './models/database';
...
async function handleChallengeRequest(event: H3Event<EventHandlerRequest>) {
  ...
  const { messageId, messageSignature, messageTimestamp } =
    validateEventVerificationHeaders(rawHeaders);
  ...
  const [webhook] = await db
    ...
    .innerJoin(tables.streamers, eq(tables.webhooks.streamerId, tables.streamers.id))
    .where(
      and(
  ...
  await db
    ...
    .where(eq(tables.webhooks.id, webhook.id));
...
async function handleNotificationRequest(event: H3Event<EventHandlerRequest>) {
  ...
  try {
    const notification: StreamOnlineNotification = v.parse(
      streamOnlineNotificationSchema,
      destructuredBody,
    );
...
export default defineEventHandler(async (event) => {
  ...
  if (messageType === 'notification') {
    await handleNotificationRequest(event);
    return;
  }

  if (messageType === 'webhook_callback_verification') {
    await handleChallengeRequest(event);
    return;
  }

  if (messageType === 'revocation') {
    await handleRevocationRequest(event);
    return;
  }
});
