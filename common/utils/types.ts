 ...
 */
type UnionToIntersect<U> = (U extends unknown ? (arg: U) => void : never) extends (
  arg: infer Intersect,
) => void
  ? Intersect
  : never;
...
type UnionToTuple<U, Result extends unknown[] = []> =
  UnionToIntersect<U extends never ? never : () => U> extends () => infer Last
    ? UnionToTuple<Exclude<U, Last>, [Last, ...Result]>
    : Result;

export type { UnionToIntersect, UnionToTuple };
