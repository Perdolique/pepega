...
<script setup lang="ts">
  ...
  const webhook = computed(() => webhooksStore.webhooks.get('stream.online'))
  const status = computed<WebhookStatus>(() => webhook.value?.status ?? 'not_active')
  const isRegisterDisabled = computed(() => isRegistering.value || webhook.value === undefined)
  ...
  async function onRemoveClick() {
    ...
    await webhooksStore.deleteWebhook('stream.online')
