...
export default defineEventHandler(async (event): Promise<ResponseData> => {
  ...
  const [webhook] = await db
    ...
    .innerJoin(tables.streamers, eq(tables.streamers.userId, userId))
    .where(
      and(eq(tables.webhooks.streamerId, tables.streamers.id), eq(tables.webhooks.id, webhookId)),
    );
  ...
  await db
    ...
    .where(eq(tables.webhooks.id, webhook.webhookId));
  ...
  try {
    ...
    const registrationResponse = await subscribeWebhook({
      appAccessToken,
