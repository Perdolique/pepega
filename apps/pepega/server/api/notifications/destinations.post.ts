...
export default defineEventHandler(async (event): Promise<NotificationDestinationModel> => {
  const { notificationId, message, telegramChannelId } = await readValidatedBody(
    event,
  ...
  const streamerNotification = db.$with('streamerNotification').as(
    db
      .select({
        id: tables.notifications.id,
      })
      .from(tables.notifications)
      .innerJoin(tables.streamers, eq(tables.streamers.id, tables.notifications.streamerId))
      .where(and(eq(tables.notifications.id, notificationId), eq(tables.streamers.userId, userId))),
  );

  // Ensure the telegram channel ID is valid, belongs to the user and verified
  const telegramChannel = db.$with('telegramChannel').as(
    db
      .select({
  ...
  const telegramProvider = db.$with('telegramProvider').as(
    db
      .select({
        id: tables.notificationProviders.id,
      })
      .from(tables.notificationProviders)
      .where(eq(tables.notificationProviders.type, 'telegram')),
  );
