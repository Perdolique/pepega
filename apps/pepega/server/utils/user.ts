...
async function getSessionUser(event: H3Event): Promise<UserModel> {
  ...
  const users = await event.context.db.query.users.findFirst({
    columns: {
      id: true,
      isAdmin: true,
      isStreamer: true,
...
async function getUserByOAuthAccount(
  ...
  const [foundUser] = await db
    ...
    .innerJoin(tables.users, eq(tables.users.id, tables.oauthAccounts.userId));

  return foundUser ?? defaultUser;
}

async function createOAuthUser({ provider, user }: OAuthUser): Promise<UserModel> {
  ...
  const newUser = await db.transaction(async (transaction): Promise<UserModel> => {
    ...
    // Create a new user
    const [foundUser] = await transaction.insert(tables.users).values({}).returning({
      id: tables.users.id,
      isAdmin: tables.users.isAdmin,
      isStreamer: tables.users.isStreamer,
    });
    ...
    await transaction.insert(tables.oauthAccounts).values({
      userId: foundUser.id,
      accountId: user.id,
    ...
    if (provider === 'twitch') {
      ...
      if (foundStreamer === undefined) {
        await transaction.insert(tables.streamers).values({
          userId: foundUser.id,
          broadcasterId: user.id,
...
export { getSessionUser, getUserByOAuthAccount, createOAuthUser };
