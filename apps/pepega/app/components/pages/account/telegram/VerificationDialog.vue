<template>
  <ModalDialog v-model="isOpen" title="Verify Telegram Channel">
    <div :class="$style.component">
      <div>
        1. To verify your channel, please add the bot
        <strong>{{ runtimeConfig.public.telegramBotName }}</strong> to your channel
        <a :href="chatUrl" target="_blank" rel="noopener noreferrer">@{{ chatId }}</a
        >.
      </div>
      ...
      <SimpleButton :disabled="isCodeSent" @click="onSendCodeClick"> Send code </SimpleButton>
      ...
      <div :class="$style.verificationControls">
        ...
        <SimpleButton :disabled="isVerifyButtonDisabled" @click="onVerifyClick">
          Verify
        </SimpleButton>
      </div>
    </div>
...
function onCodeUpdate(event: Event) {
  if (event.target instanceof HTMLInputElement) {
    const value = event.target.value.replaceAll(/\D/gu, '');
