<template>
  <div ref="rootRef" :class="$style.component">
    <button :class="[$style.chip, { active: isTooltipVisible }]" @click="toggleTooltip">
      {{ channel.chatId }}
    ...
    <AnimatePresence>
      ...
      >
        <div :class="$style.menu" ref="menuRef">
          <button
            v-if="channel.isVerified === false"
            :class="$style.menuItem"
            @click="openVerificationDialog"
          >
            <Icon name="tabler:check" size="1.25em" />

            <span :class="$style.menuItemText"> Verify channel </span>
          </button>
          ...
          >
            <Icon name="tabler:trash-x" size="1.25em" />
    ...
    >
      Are you sure you want to delete the channel "{{ channel.chatId }}"? This action cannot be
      undone.
    </ConfirmationDialog>
