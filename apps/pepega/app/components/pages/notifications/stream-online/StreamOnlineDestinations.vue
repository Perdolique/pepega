<template>
  <BaseCard :class="$style.card">
    ...
    <template v-else> No stream online notifications created yet. </template>
  </BaseCard>
</template>
...
const { state: destinations, isPending } = useQuery(() => getByNotificationId(notificationId));

const hasDestinations = computed(
  () => destinations.value.data && destinations.value.data.length > 0,
);
</script>
